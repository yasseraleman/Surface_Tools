...
for i  = 1:Nc
    ...
    if graphisdag(subGraph)
        [start_point,end_point] = find(sum(subGraph,2));
        ...
        cont = cont + 1;
        [start_point,end_point] = find(tril(subGraph) - graphminspantree(subGraph));% Selecting the start and the endpoint
...
neworder(1) = [];
reordintEdges = [intEdges(neworder,:) full(edgeId(neworder,:))];
